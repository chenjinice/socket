...
LaserServer::LaserServer(uint16_t host_port)
{	
    this->init();
...
{	
    ...
    printf("LaserServer : zmq pub = %s\n",pub_endpoint);
    m_context = zmq_ctx_new();
    m_publisher = zmq_socket(m_context,ZMQ_PUB);
    ret = zmq_bind(m_publisher,pub_endpoint);
    if(ret != 0){
        perror("LaserServer : zmq bind error");
        exit(-1);
    }
...
void LaserServer::server_send(uint8_t *buffer, int len)
{	
    if(!m_ready){
        printf("LaserServer : server is not started \n");
        return;
    }

    if(len > BUFFER_SIZE){
        printf("LaserServer : send error , len = %d > %d \n",len,BUFFER_SIZE);
        return;
    }
 ...
 *  		  0 : 时间差大于等于 ms
*/	
int LaserServer::check_interval(struct timeval *tv,int ms)
{	
...
// 发送检测物体数据
void LaserServer::send_data(laser::LaserMsg &msg,struct timeval *tv, int ms)
{
    if(msg.objs_size() == 0)return;
    // 限制一下发送频率
    if(tv != NULL){
